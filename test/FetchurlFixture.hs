...
withFetchurlServer action = Socket.withSocketsDo $ do
  ...
  let serve listener = forever $ bracket (fst <$> Socket.accept listener) Socket.close $ \client -> do
        ...
        case path of
          "/stall" -> do
            ...
            threadDelay stalledResponseMicros
          _ -> do
            void (tryPutMVar requested ())
  ...
  bracket listen Socket.close $ \listener -> do
    ...
    withAsync (serve listener) $ \worker -> do
      link worker
      action base (reverse <$> readIORef requests) (readMVar requested)
  where
...
-- The stalled response outlives the cancellation watchdog; the server is
-- cancelled with the fixture instead of making the suite wait for this delay.
stalledResponseMicros, fixtureBacklog, maxHeaderBytes, headerChunkBytes :: Int
stalledResponseMicros = 60000000
fixtureBacklog = 8
maxHeaderBytes = 8192
